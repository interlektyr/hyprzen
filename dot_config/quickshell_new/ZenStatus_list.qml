...
PanelWindow {
  ...
  property var typeStat: ZenServices.typeOfStat
  ...
  Component.onCompleted: {
    if (statusBase.typeStat === "battery" && (ZenServices.batName !== "no" || ZenServices.batName !== "")) {
  ...
  Process {
    ...
    command: ["bash", "-c", "echo $(cat /sys/class/power_supply/" + ZenServices.batName + "/capacity) $(cat /sys/class/power_supply/" + ZenServices.batName + "/status)"]
    running: ZenServices.batName === "no" || ZenServices.batName === "" ? false : true     
    stdout: StdioCollector {
    onStreamFinished: {
